...
| **开→关→再开修复** | 分析器改为**每页复用一个 `AudioContext`**（原来每次「开始」都新建、而 `close()` 是异步的 → 几次循环后构造函数抛错，界面卡在「启动中」且完全不识别）；开始前等 300 ms 让 Chrome 释放麦克风；启动失败按**退避阶梯**重试而不是 0 ms 空转；6 秒没进入 listening 就明确报错、释放麦克风并可重试；麦克风持续输出 −140 dBFS 时提示「设备被占用 / 选错输入」 |
